...
namespace py_minresqlp_wrapper
{
	...
	{
		symmetric_type(const int n, const std::vector<double>& b, const std::vector<double>& A, const parameterset& para)
	...
	{
		hermitian_type(const int n, const std::vector<dcomplex>& b, const std::vector<dcomplex>& A, const parameterset& para)
	...
	PyObject* py_minresqlp_symmetric(PyObject *numpy_b, PyObject *numpy_A, BoostPyDict info)
	{
		parameterset parameters;
		parameters.get_options(info);

		ndarray_to_C_ptr_wrapper<double> wrapperb(numpy_b), wrapperA(numpy_A);
		const int n = wrapperA.get_size(0);
		std::vector<double> b(wrapperb.get_ptr(), wrapperb.get_ptr() + n),
				    A(wrapperA.get_ptr(), wrapperA.get_ptr() + n*n);

		symmetric_type symmetric_mode(n, b, A, parameters);
		...
		return C_ptr_to_ndarray_wrapper(&(symmetric_mode.x)[0], 1, dims, NPY_FLOAT64);
	}

	PyObject* py_minresqlp_hermitian(PyObject *numpy_b, PyObject *numpy_A, BoostPyDict info)
	{
		parameterset parameters;
		parameters.get_options(info);

		ndarray_to_C_ptr_wrapper<dcomplex> wrapperb(numpy_b), wrapperA(numpy_A);
		const int n = wrapperA.get_size(0);
		std::vector<dcomplex> b(wrapperb.get_ptr(), wrapperb.get_ptr() + n),
				      A(wrapperA.get_ptr(), wrapperA.get_ptr() + n*n);

		hermitian_type hermitian_mode(n, b, A, parameters);
		...
		return C_ptr_to_ndarray_wrapper(&(hermitian_mode.x)[0], 1, dims, NPY_COMPLEX128);
	}
}
