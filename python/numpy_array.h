...
public:
	...
	long get_size(const long i) const;
	long get_ndim() const;
...
: in_con(NULL), count(1), num_dim(0), pdim(NULL), ptr(NULL)
{
	...
	for(long i=0;i<num_dim;++i) count *= pdim[i];
}
...
template <typename T>
long ndarray_to_C_ptr_wrapper<T>::get_total_size() const
{
	return count;
}

template <typename T>
long ndarray_to_C_ptr_wrapper<T>::get_size(const long i) const
{
	return pdim[i];
}

template <typename T>
long ndarray_to_C_ptr_wrapper<T>::get_ndim() const
{
	return num_dim;
}

template <typename T>
T& ndarray_to_C_ptr_wrapper<T>::operator[](const long i) const
{
	return ptr[i];
}
...
template<typename T>
PyObject* C_ptr_to_ndarray_wrapper(const T* C_ptr,const long NUM_DIMENSION, npy_intp* const DIMS, const long TYPE)
{
